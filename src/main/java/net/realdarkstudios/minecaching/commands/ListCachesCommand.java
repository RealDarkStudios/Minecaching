...
public class ListCachesCommand extends MCCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (playerCheck(sender)) {
            ...
            CacheListMenu menu = new CacheListMenu(MCMessageKeys.Menu.List.TITLE, Minecaching.getInstance(), plr);
            menu.open(plr);
        ...
        TextComponent msg;
        if (numCaches != 0) {
            msg = MCMessageKeys.Command.List.PAGE.translateComponent((page * 10) + 1, Math.min((page + 1) * 10, numCaches), numCaches);
